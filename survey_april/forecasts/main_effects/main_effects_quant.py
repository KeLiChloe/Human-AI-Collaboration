...
def aggregation_by_table_group(task_key: str, group_id: str) -> float:
    ...
    if task_key == "cos_race":
        vec_key, ml_vec = "vec_race_bin", ml_bin_race
    elif task_key == "cos_gender":
        vec_key, ml_vec = "vec_gender_bin", ml_bin_gender
    else:
...
def main() -> None:
    ...
    # ── Visualization ─────────────────────────────────────────────────────────────
    COLOR_SENIOR    = GROUP_COLORS["senior"]
    COLOR_PHD = GROUP_COLORS["phd"]
        ...
        *,
        ...
        for idx, (task_key, task_label) in enumerate(panel_defs):
            ...
            if collapsed:
                ...
                colors = [COLOR_SENIOR, COLOR_PHD, GROUP_COLORS["genai"]]
            ...
            for y_pos, grp, color in [(1, e, COLOR_SENIOR), (2, ne, COLOR_PHD), (3, ga, GROUP_COLORS["genai"])]:
                sims = grp["sims"]
            ...
            ax2.set_yticklabels(["Senior Scientists", "PhD Students", "GenAI"], fontsize=10.5)
        ...
        save_figure(out_path, pad_inches=QUANT_SUMMARY_SAVE_PAD if is_summary_panel else 0.04)
        print(f"Figure saved → {out_path}")


    def plot_sorted_cosine_individual_separate(panel="both", *, collapsed=False):
        if panel not in {"both", "race", "gender"}:
            raise ValueError(f"Invalid panel={panel!r}")
        all_panels = [
        ...
        sorted_title = "Forecasting Accuracy - Main Effects"
        if panel == "both":
            panels = all_panels
        ...
        for idx, (task_key, vec_key, ml_vec, title) in enumerate(panels):
            ...
            if collapsed:
                ...
                ax.scatter(x[is_exp], y[is_exp], c=COLOR_SENIOR, marker="o", s=52,
                           alpha=1.0, edgecolors=_edg, linewidths=_lw, zorder=5)
                ax.scatter(x[is_non], y[is_non], c=COLOR_PHD, marker="o", s=52,
                           alpha=1.0, edgecolors=_edg, linewidths=_lw, zorder=5)
        ...
        plt.tight_layout()
        if panel == "both":
            out_path = OUT_DIR / "main_effects_cosine_sorted_individuals.pdf"
        else:
            out_path = OUT_DIR / f"main_effects_cosine_sorted_individuals_{panel}.pdf"
        save_figure(out_path)
        print(f"Figure saved → {out_path}")

    import soi_quant as soi
