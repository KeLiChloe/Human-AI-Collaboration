...
def format_feature_label(feature_name, sign):
...
def plot_task(task_name, entries, output_dir):
    ...
    # Keep a robust ordering for plotting.
    ...
    df["feature_label"] = df.apply(
        lambda row: format_feature_label(row["feature"], row.get("sign", "?")),
        axis=1,
    )
    ...
    png_path = output_dir / f"{task_name}_feature_importance_votes.png"
