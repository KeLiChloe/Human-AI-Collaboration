...
Q12_TARGET_COLUMN = "Q Gender.12 post-ML theory (main effects)"
Q4_TARGET_PREFIX = "Q Gender.4"
Q12_TARGET_PREFIX = "Q Gender.12"
...
def analyze_updated_theory(q4_text, q12_text, retries=3):
    ...
    Then, after reviewing machine-learning evidence about the main effects predicting whether a paper discusses gender inequality, the respondent was asked to refine or update their theory.
...
def main():
    ...
    q12_col = resolve_column_name(df, Q12_TARGET_COLUMN, Q12_TARGET_PREFIX)

    # Insert initial-theory metrics immediately after Q Gender.4.
    df = ensure_metric_columns_after(df, q4_col, Q4_METRIC_COLUMNS)

    # Insert updated-theory metrics immediately after Q Gender.12.
    df = ensure_metric_columns_after(df, q12_col, Q12_METRIC_COLUMNS)

    for i in tqdm(
        range(len(df)),
        desc="Q Gender.4 and Q Gender.12 theory quality (API)",
        unit="row",
        mininterval=0.3,
    ):
        ...
        q4_text = df.at[i, q4_col]
        q12_text = df.at[i, q12_col]
