...
PROJECT CONTEXT
The project studies human-AI collaboration in scientific theory building. Respondents forecast whether academic papers discuss inequality-related topics, especially racial inequality and gender inequality. The survey asks respondents to construct theories about predictors of whether a paper discusses racial inequality.
...
SURVEY QUESTION OF INTEREST: Q Race.5
...
The outcome is whether a paper discusses racial inequality. Respondents may refer to the outcome as:
Y, racial inequality paper, race inequality research, probability of race inequality research, likelihood of racial inequality publication, or similar.

YOUR CODING TASK
For each respondent’s Q Race.5 response, code exactly three quantities:
...
3. number_of_latent_variables
   ...
   Do not count:
   - Y or any synonym of the racial inequality outcome
...
- female authors, share of female authors, female_score, or similar → female
- Asian authors, or similar → asian
...
- news inequality mentions, media attention, news attention to inequality, or similar  → news_inequality_mentions_3_years
- paper inequality mentions, prior publications, academic attention, recent academic attention to inequality, or similar → paper_inequality_mentions_3_years
...
def analyze_response(response_text: str, max_retries: int = 3) -> DiagramMetrics:
    ...
    user_prompt = f"""
Please analyze the following Q Race.5 diagram response.
