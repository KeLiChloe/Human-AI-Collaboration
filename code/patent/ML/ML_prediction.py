...
def load_and_prepare_data(file_path):
    ...
    df['target'] = df["label"].apply(lambda x: 1 if int(x) == 1 else 0)
    ...
    drop_columns = ['target', 'label',
                    'patent_title', 'patent_abstract',
                    'news_ineq_t-0', 'news_ineq_t-1', 'news_ineq_t-2', 'news_ineq_t-3', 'news_ineq_3yr_avg',
                    'patent_ineq_t-0', 'patent_ineq_t-1', 'patent_ineq_t-2', 'patent_ineq_t-3', 'patent_ineq_3yr_avg',
                    'year',
                    ]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python ML_prediction.py <file_path> <model_save_dir>")
        sys.exit(1)
